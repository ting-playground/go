...
func waitIfSelected(c *hchan, recv bool, scases []scase, lockorder []uint16) bool {
	if fastrandn(100) > c.selectProbability {
		return false
	}
	...
	// we are selecting this c, we have to wait c if it doesn't ready
	timeSleep(int64(c.selectWaitTimeout))

	sellock(scases, lockorder)
	return true
}

func typeSelect(cas0 *scase, order0 *uint16, pc0 *uintptr, nsends, nrecvs int, block bool) (int, bool) {
	if debugSelect {
	...
	for _, casei := range pollorder {
		...
		if !selected {
			selected = waitIfSelected(c, casi >= nsends, scases, lockorder)
		}
